...
- [ ] Create the full directory structure per PDR Section 4.1
...
- [ ] Create `pyproject.toml` (or `setup.py`) with project metadata and dependency list
- [ ] Set up a virtual environment and pin initial dependencies
- [ ] Configure linting/formatting (ruff or black + isort)
- [ ] Configure pytest and establish `tests/` conventions

### 0.2 Data Models

- [ ] Implement `core/models.py` — define dataclasses/Pydantic models for:
...
- [ ] Add enum definitions for content_type, source kind, and trust_label
- [ ] Write unit tests for model creation and validation

### 0.3 SQLite Storage Layer

- [ ] Implement `storage/sqlite.py` — database connection manager
  - Enforce WAL mode, foreign keys ON
  - Provide a context-managed connection/session interface
- [ ] Create initial migration in `storage/migrations/` for tables:
  - `notes`
  - `sources`
  - `signals`
  - `audit_log`
- [ ] Implement a lightweight migration runner (ordered SQL scripts)
- [ ] Write tests: schema creation, foreign key enforcement, WAL mode verification

### 0.4 Core Services — Notes & Sources

- [ ] Implement `core/services/notes.py`
...
- [ ] Implement `core/services/signals.py`
...
- [ ] Write tests for note CRUD and signal lifecycle

### 0.5 Audit Service

- [ ] Implement `core/services/audit.py`
...
- [ ] Write tests for audit log append and retrieval

### 0.6 Full-Text Search (FTS)

- [ ] Add FTS5 virtual table for notes content in a migration
- [ ] Wire FTS into `notes.search_notes(query)`
- [ ] Write tests: indexing on insert, search relevance, empty results

### 0.7 CLI — Capture Commands

- [ ] Implement `cli/main.py` using Click (or argparse)
- [ ] `add` command — accepts text (stdin or argument), optional tags, source kind
  - Calls IngestionAgent (or directly calls note/source services for Phase 0)
  - Prints created note ID
- [ ] `search` command — keyword query, prints matching notes with IDs and snippets
- [ ] `show` command — display full note by ID with metadata
- [ ] Write integration tests for each CLI command

### 0.8 IngestionAgent (Basic)

- [ ] Implement `agents/ingestion.py`
  - Create Source record
  - Create Note record (immutable)
  - Extract tags/entities via regex + keyword list
  - Compute content_hash
  - Emit `signal:new_note`
- [ ] Write tests for the full ingestion pipeline

### 0.9 Phase 0 Validation

- [ ] End-to-end test: add a note via CLI, search for it, show it, verify audit log
- [ ] Verify all state survives process restart (persistence test)
- [ ] Verify no in-memory-only state exists
...
- [ ] Add enum definitions for belief status, decay_model, edge from/to types, rel_type
- [ ] Write unit tests for model creation and validation

### 1.2 SQLite Schema — Beliefs & Edges

- [ ] Create migration for `beliefs` table
- [ ] Create migration for `edges` table with composite foreign-key-like referential checks
- [ ] Write tests: schema constraints, referential integrity

### 1.3 Core Services — Beliefs & Edges

- [ ] Implement `core/services/beliefs.py`
...
- [ ] Implement `core/services/edges.py`
  ...
  - `delete_edge(edge_id)`
- [ ] Write tests for belief CRUD, status transition enforcement, edge CRUD

### 1.4 Confidence & Decay Rules

- [ ] Implement `core/rules/confidence.py`
...
- [ ] Write tests: confidence calculation with various edge configurations, decay over time

### 1.5 Contradiction Rules

- [ ] Implement `core/rules/contradictions.py`
...
- [ ] Write tests for contradiction detection heuristics
...
- [ ] Update IngestionAgent to compute and store embeddings on note creation
- [ ] Write tests: embedding storage, similarity search, index rebuild
...
- [ ] Write tests for the full synthesis pipeline

### 1.8 ChallengerAgent

- [ ] Implement `agents/challenger.py`
...
- [ ] Write tests for challenge detection and state transitions

### 1.9 Ask Pipeline

- [ ] Implement ask pipeline (in `cli/main.py` or a dedicated service)
...
- [ ] Add `ask` CLI command
- [ ] Write tests: ask with known data returns grounded answer, uncited claims are rejected

### 1.10 Phase 1 Validation

- [ ] End-to-end test: add multiple notes, verify beliefs are proposed, contradictions detected
- [ ] End-to-end test: ask a question, verify answer cites stored evidence
- [ ] Verify all belief transitions are logged in audit trail
- [ ] Verify confidence recomputation after new evidence
